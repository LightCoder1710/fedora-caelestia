            ...
            ListView { // App results
                ...
                model: ScriptModel {
                    ...
                    values: { // Search results are handled here
                        ...
                        if (root.searchingText.startsWith(ConfigOptions.search.prefix.clipboard)) { // Clipboard
                            ...
                            if (searchString === "wipe" || searchString === "clear") {
                                return [{
                                    name: qsTr("Wipe entire clipboard history"),
                                    clickActionName: qsTr("Wipe"),
                                    type: qsTr("Action"),
                                    materialSymbol: 'delete_forever',
                                    execute: () => {
                                        Hyprland.dispatch("exec cliphist wipe");
                            ...
                            return Cliphist.fuzzyQuery(searchString).map(entry => {
                                return {
                                    ...
                                    clickActionName: qsTr("Paste"),
                                    type: `#${entry.match(/^\s*(\S+)/)?.[1] || ""}`,
                                    execute: () => {
                                        Hyprland.dispatch(`exec echo '${StringUtils.shellSingleQuoteEscape(entry)}' | cliphist decode | wl-copy`);
                                        Hyprland.dispatch(`exec sleep 0.15 && hyprctl dispatch sendshortcut CTRL, V`);
                                    }
                                };
                            }).filter(Boolean);
                        } 
                ...
                delegate: SearchItem { // The selectable item for each search result
                    ...
                    query: root.searchingText.startsWith(ConfigOptions.search.prefix.clipboard) ? 
                        ...
                        root.searchingText;
                    onDeleteRequested: (cliphistStr) => {
                        Hyprland.dispatch(`exec echo '${StringUtils.shellSingleQuoteEscape(cliphistStr)}' | cliphist delete`);
                        root.refreshTrigger += 1;
                    }
                }
            }
