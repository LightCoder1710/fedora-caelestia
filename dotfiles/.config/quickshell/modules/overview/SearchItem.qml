...
RippleButton {
    ...
    property var onDeleteRequested
    ...
    Keys.onPressed: (event) => {
        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
            ...
            if (root.cliphistRawString && root.onDeleteRequested) {
                root.onDeleteRequested(root.cliphistRawString);
                event.accepted = true;
            }
        }
    }
        ...
        Component {
            ...
            IconImage {
                ...
                property string primarySource: {
                    if (!root.itemIcon) return "";
                    if (root.itemIcon.startsWith("/") || root.itemIcon.startsWith("file:")) return root.itemIcon;
                    return Quickshell.iconPath(root.itemIcon);
                }

                source: primarySource !== "" ? primarySource : "file:///usr/share/icons/Papirus/48x48/mimetypes/application-x-executable.svg"
                width: 35
                height: 35

                onStatusChanged: {
                    if (status === Image.Error) {
                        if (source == "file:///usr/share/icons/Papirus/48x48/mimetypes/application-x-executable.svg") {
                            source = "file:///usr/share/icons/Adwaita/scalable/mimetypes/application-x-executable.svg";
                        } else if (source != "file:///usr/share/icons/Adwaita/scalable/mimetypes/application-x-executable.svg") {
                            source = "file:///usr/share/icons/Papirus/48x48/mimetypes/application-x-executable.svg";
                        }
                    }
                }
            }
